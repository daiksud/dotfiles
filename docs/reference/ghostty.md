...
The following terminal-level translations preserve the Emacs editing actions used
by zsh while avoiding conflicting application shortcuts.
...
These mappings apply to every program running in Ghostty, while preserving the
usual shell editing semantics.
