...
`install.sh` validates `install_map.json`, creates ordinary links, runs the
numbered setup scripts, and generates the Git SSH `allowed_signers` file when
its inputs exist. See
